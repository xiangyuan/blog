...
def build():
    ...
    local('find _build -type f -exec sed -i "s/\(link.*\)style.css/\\1style.css?1371592067?%s/g" {} \;' % int(time.time()))
    local('find _build -type f -exec sed -i "s/\(link.*\)_pygments.css/\\1_pygments.css?1371592069?%s/g" {} \;' % int(time.time()))
